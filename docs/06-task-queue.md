...
**Status:** SCHEDULER + EXACT REAL ARTIFACT PIPELINE + ACTUAL BROWSER MODULE-WORKER STRUCTURAL PASS + BYTE-LIFECYCLE/OPT-IN RESIDENT CAP PASS / EXACT REAL BROWSER + ANDROID MOVEMENT OPEN  
...
**STRØM byte-lifecycle evidence:** the scheduler previously documented `maxCacheBytes` as an inactive-cache budget while its counter also included resident payloads. `agent/strom-budget-accounting` separates resident/activating/cached payload bytes, keeps `maxCacheBytes` scoped to inactive cache, and adds an opt-in `maxResidentBytes` hard activation/resident cap that defaults to `null` rather than selecting a production value. Local Node 22 regressions pass **11 cases**, including resident bytes not evicting valid warm cache, resident-budget deferral followed by cache-backed activation without refetch, activation failure retry, disposal failure preserving accounting, cancellation/stale completion and transport retry. A synthetic 3×3 pressure run holds final resident/cache overcommit at **0 B**, peak concurrency at **2**, and records 4 cache hits / 9 resident-budget deferrals / 6 evictions under deliberately constrained test-only limits.  
...
**Proof:** `docs/proofs/2026-08-18-terrain-runtime-pipeline.md`, `docs/proofs/2026-08-18-browser-provenance-parity.md`, `docs/proofs/2026-08-18-world-viewer-terrain-worker.md` and `docs/proofs/2026-08-18-strom-streaming-budget-accounting.md`.  
**Open:** drive the exact accepted Nannestad terrain bytes through this real browser-worker path; then measure Android transfer/startup, verification/decode, GPU upload/apply, rAF gaps and movement. Real 2×2/3×3 terrain remains blocked by `P0-MULTITILE-TERRAIN-01`. Production resident/cache/GPU values, worker pooling, verification caching and LOD remain unselected until device evidence exists. The scheduler byte cap measures renderer-neutral retained payload bytes, not exact VRAM.  
**Next:** reuse the deployed exact-real Preview-1 path for a controlled movement/exit/return Android Chrome capture with scheduler bytes + verification/decode/worker/GPU/rAF timings. If main-thread jank remains, isolate decode and provenance work before choosing a worker/cache policy.
