...
**Status:** CANDIDATE WORLD↔TILE↔RENDER CONTRACT + ADVERSARIAL NODE PASS / WHOLE-NORWAY + DEVICE/PHYSICS POLICY OPEN  
**Owner area:** `engine/world`, future simulation/runtime boundary  
**Done/evidence:** merged isolated precision/origin experiments still establish the numerical direction: absolute EPSG:25832-scale Float32 is too coarse while high-precision world state can remain independent of disposable render-local Float32. ATLAS now adds a production-direction candidate contract in `engine/world`: explicit `WorldFrame` with separate horizontal CRS and vertical datum, authoritative Float64 `WorldPosition`, high-precision `TileFrame` anchors, Float32 render-local derivation only after Float64 subtraction, render-frame identity as `(originSeriesId, epoch)`, historical origin reconstruction/expiry, deterministic authoritative snapshot serialization that excludes render-origin state, and a renderer-independent `SubsystemLocalFrame` boundary for future physics/local engines. The new 8-case Node regression covers origin shift mid-tick, temporal delta over epoch boundary, large absolute coordinates, tile-boundary crossing, entity crossing the render anchor, serialization/replay, 1,000 render-origin shifts with an independent physics-local adapter, and fail-closed frame/datum/epoch/series mismatches. Focused observations: **0.024125 mm** max world reconstruction error in the exercised large-coordinate case, **0.085877 m** error when the same absolute northings are naïvely converted to Float32 before subtraction, and **0.048829 mm** max compensated physical-delta error across the exercised mid-tick shift. Proof: `docs/proofs/2026-08-18-atlas-world-coordinate-contract.md`.  
**Prior scale/performance evidence:** deterministic origin experiment: **2,048 entities / 3,600 ticks / 29 shifts**, identical authoritative Float64 state and max local reconstruction error **0.244141 mm**; hosted Node/V8 directional benchmark at 100k entities measured median ~0.279 ms Float64 integration and ~0.218 ms Float32 local derivation/rederive. These remain Node evidence, not Android/browser/GPU/physics acceptance.  
**Open/next:** do not promote `projected-cartesian-height`, EPSG:25832, any render-origin anchor or any shift threshold to whole-Norway policy from these tests. Have LUMEN/STRØM consume the candidate contract on the exact-real browser/device camera+tile path, then measure realistic camera/player movement, origin shifts, temporal history behavior and rebinding cost. ATLAS must separately test an actual physics adapter and later network snapshot/quantization boundary before a final coordinate policy can enter `docs/04-decisions.md`.
