...
**Profiler:** active LUMEN PR #50 adds a deployable exact-artifact browser profiler that keeps normal production verification intact, then replays the same verifier on already-fetched bytes and separates strict JSON decode. First replay is reported separately from later steady-state samples so one-time browser/JIT/crypto warm-up is not silently used as cache/worker evidence.  
...
**Next:** obtain exact-head automated browser measurements from the profiler and compare first-replay vs steady-state verification/decode cost. Only if repeated cost is material should STRØM/SENTINEL evaluate worker placement or verification caching; mandatory full verification semantics remain unchanged.
