...
**Status:** NODE + REAL CHROME FULL-GRAPH PARITY PASS / ANDROID EXACT-TERRAIN OBSERVATION PASS / POLICY OPEN  
**Owner area:** `engine/schemas`, `engine/streaming`, browser artifact consumer  
**Done/evidence:** strict JSON Schema 2020-12 contracts and producer compatibility are merged. Provenance semantics now live once in crypto-agnostic `runtime_verifier_core.mjs`; Node and browser adapters only calculate RFC 8785/JCS + SHA-256 hashes. Hosted parity run `32136500278` requires identical Node/WebCrypto decision, code and reconstructed hashes across all **11** existing happy/adversarial cases. Real Chrome run `32136951610` reconstructs complete provenance graphs before decode with **0 raw-source calls**. Operator Android device evidence on exact accepted Nannestad terrain/roads/buildings also reports all three `RUNTIME_VERIFICATION_PASS`, 7 runtime requests / 0 raw-source calls; the exact terrain pipeline measured ~**4.9 ms** verification in the diagnostic WebGPU balanced capture.  
**Performance observation:** separate hosted runs moved road/building `verify_decode_ms` from roughly **112.9 / 89.8 ms** before full browser graph verification to **201.7 / 173.4 ms** with it; boot-to-first-visible moved ~788.5 -> ~993.5 ms. Those runs are not controlled crypto-only A/B. The Android terrain verification observation is likewise one capture, not a caching/worker policy decision. Security semantics remain mandatory.  
**Proof:** `docs/proofs/2026-08-18-browser-provenance-parity.md`, `docs/proofs/2026-08-18-world-viewer-terrain-worker.md`, `docs/proofs/2026-08-18-sentinel-streaming-device-integration.md`.  
**Next:** preserve full graph reconstruction; only compare worker-side verification or immutable-identity verification caching if repeated movement/multi-tile device evidence shows responsiveness pressure.
...
**Status:** EXACT-REAL ANDROID WEBGL2 + DIAGNOSTIC WEBGPU + STRICT MATCHED A/B PASS / NORMAL-CHROME WEBGPU + GPU-TIMESTAMP + REPEATABILITY OPEN  
**Done/evidence:** the deployable Vite World Viewer consumes the exact accepted Nannestad terrain/road/building artifacts with full browser provenance, actual module DedicatedWorker, renderer-neutral scheduler/cache probe and backend measurement. Operator Android Chrome WebGL2 and diagnostic Unsafe-WebGPU captures now have a strict matched pair on commit `b8cb6b35de3847aaab4357a9d89f81029dfb6997`, capture session `ab-msaa1-001`, `balanced`, DPR 1.5, one-sample raster workload, identical 360×447 CSS / 540×642 backing surface, camera, artifacts, streaming contract and 90-frame measurement window. Reconstructed `compareDeviceEvidenceContext()` returns **0 mismatches / comparable=true**. Both captures have 7 runtime requests / 0 raw-source calls and exact accepted provenance. WebGPU obtains the **core** adapter directly when Unsafe WebGPU is enabled.  
**Strict A/B observation:** WebGPU repeated-draw CPU was **0.300 / 0.600 / 0.655 / 1.100 ms p50/p95/p99/max** versus WebGL2 **1.150 / 1.855 / 2.985 / 6.100 ms**, reductions of ~**73.9% / 67.7% / 78.1% / 82.0%** in CPU-side draw cost. WebGPU GPU-resource-apply CPU was **2.7 ms** vs WebGL2 **19.2 ms** and renderer init **31.4 ms** vs **53.9 ms**. Frame-gap p50 was effectively identical at 16.7 ms; WebGPU p95 was slightly worse (18.055 vs 16.855 ms) while p99/max were lower (18.577/19.2 vs 35.315/50.0 ms). This is one 90-frame capture, not a statistical/general-device conclusion.  
**Startup boundary:** do **not** attribute first-frame total to renderer backend from this pair. WebGPU `input_to_first_frame_ready` was 1081.7 ms vs WebGL2 487.2 ms, but the WebGPU pre-render terrain pipeline was 496.4 ms vs 236.2 ms; `resolveInput` and worker roundtrip varied by +160.3/+98.7 ms before renderer initialization. A repeated alternating/cold-warm or artifact-local startup experiment is required before assigning first-visible latency to WebGPU/WebGL2.  
**Critical limitations:** Unsafe WebGPU is diagnostic only; normal Chrome remains adapter-blocked on this observed handset/browser state. `draw_cpu` measures CPU-side command/update work, not GPU execution. WebGPU reports timestamp-query capability but GPU timestamps are not yet captured. Repeated fixed-scene draws are not gameplay/camera-motion acceptance. No final WebGPU/WebGL2/Cesium architecture decision is made.  
**Proof:** `docs/proofs/2026-08-18-world-viewer-terrain-worker.md`, `docs/proofs/2026-08-18-sentinel-streaming-device-integration.md`, `docs/proofs/2026-08-19-android-webgpu-webgl2-strict-ab.md`.  
**Next:** add capability-gated actual GPU timing (WebGPU timestamp-query and closest feasible WebGL2 timer-query equivalent), run repeated alternating backend captures to characterize p95/p99 variance, and isolate startup/network/runtime phases before comparing first-visible. Keep WebGL2 fallback while normal Chrome WebGPU availability remains blocked.  
**Parallel issue:** GitHub Issue #5 owns viewer batching/performance.

### P0-STREAMING-01 — Verified terrain tile lifecycle
**Status:** EXACT-REAL BROWSER + ANDROID SINGLE-TILE MOVEMENT PASS / GPU RESOURCE LIFECYCLE + REAL MULTI-TILE OPEN  
**Owner area:** `engine/streaming`  
**Done/evidence:** `TileStreamingScheduler`, deterministic terrain mesh worker boundary and `terrain_tile_loader.mjs` are merged. The exact accepted 4,000,382 B Nannestad terrain artifact is now proven through full browser verification, strict NWEHGT01 decode, actual module DedicatedWorker and scheduler lifecycle. Operator Android device evidence on the exact Nannestad graph proves `resident -> cached -> resident` with 1000 m movement, 800/1200 m active/retain radii, resolver calls **1 -> 1**, load-start delta **0**, cache-hit delta **1**, trace **9 retained / 0 dropped**, one completed terrain load and **4,729,120 B** retained. The diagnostic WebGPU balanced capture measured terrain pipeline **271.8 ms** total: 104.5 ms resolve, 4.9 ms verify, 21.0 ms decode, 141.0 ms worker roundtrip / 28.6 ms worker-reported.  
**Critical boundary:** `renderer_resource_lifecycle_observed=false`; the Preview renderer keeps the already-created terrain GPU resource while the scheduler moves the tile between resident/cached states. This proves runtime/cache lifecycle, not GPU unload/reload. Real 2×2/3×3 terrain remains blocked by `P0-MULTITILE-TERRAIN-01`. Hard resident/GPU budgets, worker pooling, verification caching and LOD remain unselected.  
**Proof:** `docs/proofs/2026-08-18-terrain-runtime-pipeline.md`, `docs/proofs/2026-08-18-browser-provenance-parity.md`, `docs/proofs/2026-08-18-world-viewer-terrain-worker.md`, `docs/proofs/2026-08-18-sentinel-streaming-device-integration.md`.  
**Next:** implement/measure renderer GPU-resource activation/deactivation separately from scheduler cache state. In parallel, do not start real neighboring-tile runtime acceptance until FORGE closes the DTM1 seam-authority gate.
