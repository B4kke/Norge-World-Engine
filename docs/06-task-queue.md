...
**Open:** the full graph-reconstructing verifier currently depends on Node `crypto`; the browser viewer has WebCrypto byte/size verification but not the same full JCS graph reconstruction. A browser-compatible full verifier is therefore still required before the web client can claim parity with the Node runtime gate.
...
**Status:** ANDROID TERRAIN/VECTOR PASS + HOSTED FIXED-CAMERA BATCHING PASS / ANDROID BATCHING GATE OPEN  
**Done now:** Forsøk 16 SHA-verifies and renders the real terrain, road and building artifacts with 0 raw-source calls. The 1 m / 1000×1000 DTM1 artifact remains world truth while the mobile GPU terrain is sampled to 129×129. Android screenshot evidence reports **1.3 ms terrain decode, 19.4 ms terrain mesh build, 220 ms boot, 224 draw calls, 16.7 ms / 60 FPS, 382 geometries and 2 textures** at the captured oblique camera. A separate hosted fixed-camera real-artifact batching harness on PR #8 uses 246 road paths + 135 building footprints (381 logical vector objects) and reduces submissions **381 -> 2 draw calls** while keeping raw-source runtime calls at 0. Fresh post-QA hosted evidence reports frame p95 **33.4 -> 16.7 ms** and render-sync p95 **0.6 -> 0.2 ms**.  
**Important limitation:** hosted/headless timing is comparative harness evidence, not an Android GPU acceptance result. PR #8 remains an unmerged draft only because the connected GitHub draft→ready mutation was blocked; its baseline and dedicated realdata viewer-benchmark gates are green.  
**Next highest-value work:** clear/merge #8 when the draft state can be changed, then run the same fixed-camera before/after batching harness on Android. Do not select WebGL/WebGPU/Cesium from hosted timing alone.  
...
**Status:** SCHEDULER + WORKER MERGED / VERIFIED-ARTIFACT PIPELINE SYNTHETIC PASS / ACCEPTED REAL TERRAIN PIPELINE GATE OPEN  
**Owner area:** `engine/streaming`  
**Done/evidence:** renderer-independent `TileStreamingScheduler` provides deterministic camera-distance priority, active/retain radii, max resident count, max concurrent loads, resident↔cached lifecycle, inactive-cache byte budget/eviction, load aborts, stale-completion rejection, failure retry and lifecycle metrics. The merged Dedicated Worker boundary generates deterministic renderer-neutral terrain mesh buffers and returns elevation ownership. New `terrain_tile_loader.mjs` composes `RuntimeVerificationBundle + artifact bytes -> full verification -> strict NWEHGT01 semantic decode -> worker -> scheduler payload` without importing a renderer or choosing an origin policy. Hosted baseline passes **7 new adversarial pipeline cases**: canonical decode, truncation/trailing-byte rejection, tampered SHA rejection before worker, semantically invalid but hash-bound artifact rejection before worker, tile-ID mismatch, pre-abort zero-work, and an end-to-end scheduler resident load. Retained byte accounting covers the returned elevation grid + generated mesh rather than discarded transport bytes.  
**Important limitation:** the new integration regression uses a structurally exact synthetic NWEHGT01 bundle/artifact. It does not yet prove the accepted 4,000,382 B Nannestad artifact through this exact loader/worker/scheduler composition, browser full-provenance parity, HTTP/cache latency, Android worker roundtrip, GPU upload, frame gaps, seams or LOD. The real 3×3 motion gate also remains blocked by `P0-MULTITILE-TERRAIN-01`.  
**Proof:** implementation baseline run `32133954937` on head `9498afdc3a06fdba37740ba0b7b2bc4908e3c7e0`; add a persistent proof note before merge.  
**Next:** drive the exact accepted single-tile Nannestad terrain artifact/bundle through this pipeline in hosted CI without committing the 4 MB artifact or 1.1 GB raw source. Then add browser-compatible full provenance reconstruction and Android movement/device metrics. Only after that should worker pooling, hard resident/GPU budgets or LOD policy be selected.
