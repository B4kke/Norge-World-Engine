...
**Proof:** `docs/proofs/2026-08-18-browser-provenance-parity.md`, `docs/proofs/2026-08-18-world-viewer-terrain-worker.md`, and LUMEN PR #27 / run `32167399718`.  
**Next:** measure full provenance verification on Android Chrome using the exact accepted Preview 1 inputs. If device evidence shows main-thread responsiveness/first-visible pressure, compare provenance verification in a worker and/or caching of already-verified immutable identities; do not weaken graph reconstruction.
...
**Status:** EXACT REAL PREVIEW WEBGL2 HOSTED OBSERVABILITY PASS / HOSTED WEBGPU CONTROL UNAVAILABLE / ANDROID SAME-DEVICE GATE OPEN  
**Owner area:** `apps/world-viewer`, renderer/browser platform  
**Done/evidence:** Forsøk 16 renders the verified terrain/road/building artifacts on Android with 0 raw-source calls. The merged fixed-camera vector harness proves 381 logical vector objects → 2 vector draw calls. LUMEN PR #27 now drives the **exact accepted Preview 1 terrain + road + building artifacts** through the actual deployable World Viewer and instruments first-frame, startup rAF, repeated draw percentiles, draw CPU/calls, resource apply, GPU buffer payload, retained bytes and browser memory. Run `32167399718` for head `0a113d383d1b7a00d05c27094ec17d9491d9cab9` is PASS together with baseline `32167399717` and viewer-benchmark `32167399817`. The exact WebGL2 run matches terrain SHA `780de19ef1c7911bcf2476def2b91dee078612b11d10ef62923c411c6679bd96`, road SHA `34b9cd4594230df111f4563ee79e6d0a919c1c33be3502dbbcadf1afa5a6db8a` and building SHA `678c59603fba2b66d93e7a2252a3c3260a3d80d6a1da0db2c235b9c71423f7cd`, performs exactly 7 bundle/artifact requests and **0 raw-source runtime calls**, and renders 16,641 terrain vertices / 32,768 triangles, 246 road paths and 135 footprints at **4 draw calls/frame**.  
**Hosted WebGL2 measurement:** input→first-frame-ready **1961.4 ms**; startup rAF p50/p95/p99/largest **16.7 / 347.515 / 512.654 / 533.3 ms**. The opt-in 90-frame identical-scene redraw window measured frame-gap p50/p95/p99/largest **50.1 / 110.845 / 151.915 / 166.6 ms** and draw-CPU p50/p95/p99/largest **54.5 / 77.165 / 162.136 / 168.9 ms**. Resource evidence: scene build 44.1 ms, GPU resource apply 29.6 ms, renderer init 625.2 ms, 9 GPU buffers / 849,246 B logical buffer payload and 4,729,120 B retained terrain. These values are hosted/headless directional diagnostics, not Android/GPU acceptance.  
**WebGPU limitation:** a minimal 64×64 control clear/submit/present probe fails on the hosted runner with `A valid external Instance reference no longer exists.` The benchmark therefore reports `PARTIAL` for renderer comparison and deliberately does **not** claim WebGPU-vs-WebGL2 performance. A real WebGPU transport defect found earlier by the gate — non-4-byte-aligned `GPUQueue.writeBuffer` payloads for odd Uint16 index counts — is fixed and regression-covered without altering logical geometry/artifact identity.  
**Evidence artifact:** Actions artifact `world-viewer-renderer-benchmark-fad7a44272558b5f0ab7bb9c38e6773099c4705e` (id `9335952780`).  
**Next:** run the exact same accepted artifacts, camera and graphics profile on Android Chrome with forced WebGL2 and forced WebGPU where supported. Capture first-frame, frame p50/p95/p99/largest, draw calls, upload/apply, retained/memory and timestamp-query data where practical. Do not select WebGPU/WebGL2/Cesium from hosted evidence. Retry exact-commit Vercel smoke when deployment capacity permits.  
**Parallel issue:** GitHub Issue #5 owns viewer batching/performance.

### P0-STREAMING-01 — Verified terrain tile lifecycle
**Status:** SCHEDULER + EXACT REAL ARTIFACT PIPELINE + EXACT REAL BROWSER MODULE-WORKER INTEGRATION PASS / ANDROID MOVEMENT OPEN  
**Owner area:** `engine/streaming`  
**Done/evidence:** `TileStreamingScheduler`, deterministic terrain mesh worker boundary and `terrain_tile_loader.mjs` are merged. Main run `32134507528` drove the exact accepted 4,000,382 B Nannestad terrain artifact through full runtime verification, strict NWEHGT01 decode, `TerrainMeshWorkerClient` protocol path and scheduler resident lifecycle using an in-process protocol worker shim. Forsøk 18 separately proved the actual browser module DedicatedWorker lifecycle on a synthetic fixture. LUMEN integration run `32167399718` now composes the exact accepted Nannestad terrain RuntimeVerificationBundle/artifact through the same browser `verifyRuntimeBundleWeb -> terrain_tile_loader -> TerrainMeshWorkerClient -> TileStreamingScheduler` path before renderer resource creation, retaining **4,729,120 B**, with the accepted terrain identity and 0 raw-source runtime calls. This is cross-boundary integration evidence only; scheduler/worker/cache policy ownership remains with STRØM.  
**Open:** Android transfer/startup, verification/decode, GPU upload/apply, rAF gaps and camera movement remain required. Real 2×2/3×3 terrain remains blocked by `P0-MULTITILE-TERRAIN-01`. Hard resident/GPU budgets, worker pooling, verification caching and LOD remain unselected until device evidence exists.  
**Next / STRØM handoff:** reuse this exact-real Preview 1 browser path on Android Chrome and reconcile movement/cache lifecycle under device metrics. If main-thread jank remains, isolate decode/provenance cost before choosing a worker/cache policy; LUMEN should not take over scheduler ownership.
