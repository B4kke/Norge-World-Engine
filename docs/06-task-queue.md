...
**Status:** HIGHEST OPEN TERRAIN GATE / MULTI-TILE FOUNDATION MERGED / PROVIDER MOSAIC EVIDENCE FOUND / AUTHORITY STILL OPEN  
**Owner area:** `engine/compiler`, `engine/streaming`  
...
**Critical blocker:** the two authoritative raw files overlap by 10 m but their valid heights are not identical. The current production mosaic deliberately rejects the overlap instead of choosing first/newest/mean/tolerance. Dedicated diagnostics compare candidate surfaces to Kartverket's seamless 1 m WCS only as an independent QA sensor; WCS is not admitted as source authority.  
**New authority evidence:** Kartverket/Høydedata's live `NHM_DTM_25833` ImageServer catalog exposes logical raster items `33-125-116` and `33-125-117`; their provider catalog footprints also overlap by exactly 10 m. The service publishes default `ByAttribute` + `sortField=NAME` + ascending + `First` + bilinear mosaic configuration. This is not yet a production seam rule: the provider declares `NAME` as a string field while ArcGIS REST documents `ByAttribute` ordering for numeric/date fields, and matching catalog name/geometry does not prove byte identity with the SHA-addressed Atom GeoTIFFs or explicitly scope the ImageServer default rule as authority for downloadable DTM1 overlap. FORGE therefore classifies this evidence `FAIL_CLOSED_UNPROVEN`.  
**Proof:** `docs/proofs/2026-08-18-dtm1-seam-authority-probe.md`; live metadata-only gate: `.github/workflows/dtm1-seam-authority-probe.yml`.  
...
**Next:** seek explicit provider scope/ordering semantics or stronger machine-readable identity evidence; only after those blockers close may the seam TransformContract be versioned and the real 3×3 promotion gate run.  
