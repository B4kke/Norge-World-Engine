...
**Status:** EXACT-REAL ANDROID WEBGL2 PASS + DIAGNOSTIC WEBGPU PASS / NORMAL-CHROME WEBGPU + STRICT A/B OPEN  
**Done/evidence:** the deployable Vite World Viewer now consumes the exact accepted Nannestad terrain/road/building artifacts with full browser provenance, actual module DedicatedWorker, renderer-neutral scheduler/cache probe and backend measurement. Operator Android Chrome WebGL2/high capture passed with exact artifacts, 7 runtime requests / 0 raw-source calls, `resident -> cached -> resident`, 90 repeated draws and 4 draw calls/frame. On Chrome 151's normal state, `navigator.gpu` exists but both core and compatibility adapter requests return null; `chrome://gpu` evidence classifies the available Xclipse 960 Vulkan and OpenGL ES compatibility adapters as blocklisted. With Chrome's Unsafe WebGPU diagnostic flag enabled, build `a5f2e65d...` completes the exact-real Preview 1 path on active **WebGPU** with no fallback: first-frame-ready **386.3 ms**, repeated 90-draw frame gap p50/p95/p99 **16.7 / 18.255 / 20.548 ms**, draw CPU p50/p95 **0.30 / 0.555 ms**, 4 draw calls, 13 GPU buffers, 849,566 B buffer payload and timestamp-query support reported.  
**Important limitation:** the Unsafe WebGPU run is diagnostic only; it does not prove normal-production Chrome availability. Existing WebGL2 and WebGPU device captures use different commits/profiles/sessions/render surfaces and therefore are **not** strict A/B evidence. The first successful WebGPU PASS also dropped its core-vs-compatibility adapter-mode field; evidence plumbing has been corrected and requires a fresh capture for that detail. Repeated identical-scene draws remain non-gameplay evidence.  
**Proof:** `docs/proofs/2026-08-18-world-viewer-terrain-worker.md`, `docs/proofs/2026-08-18-sentinel-streaming-device-integration.md`.  
**Next:** on the corrected device-evidence head, take one fresh WebGPU diagnostic capture and one WebGL2 capture with the same commit, capture session, `balanced` profile, surface/camera and 90-frame window; require `compareDeviceEvidenceContext().comparable=true` before interpreting backend performance. Do not select WebGPU/WebGL2/Cesium from the unsafe run alone.  
