...
assert.equal(renderer.toneMappingExposure, 0.96);
...
assert.equal(lighting.sun.intensity, 2.35);
...
assert.equal(lighting.snapshot().shadow.intensity, 0.58);
assert.equal(lighting.snapshot().shadow.normal_bias, 0.06);
...
assert.deepEqual(lighting.sun.position.toArray(), [-30, 151, 5]);

const mesh = new THREE.Mesh(new THREE.BoxGeometry(1, 1, 1), new THREE.MeshStandardMaterial());
configureMeshShadowRole(mesh, { cast: true, receive: true });
assert.equal(mesh.castShadow, true);
assert.equal(mesh.receiveShadow, true);
configureMeshShadowRole(mesh, { cast: false, receive: true });
assert.equal(mesh.castShadow, false, 'receive-only shadow role must be representable for coarse terrain');
assert.equal(mesh.receiveShadow, true);
