...
function assertBrowserResult(report, manifest, serverRequests) {
  ...
  if (result?.schema !== 'nwe.world-preview-runtime/0.1' || result.status !== 'PASS') throw new Error(`runtime result invalid: ${result?.schema}/${result?.status}`);
  if (result.tile_id !== manifest.tile.id) throw new Error(`tile mismatch ${result.tile_id} != ${manifest.tile.id}`);

  const expected = { terrain: manifest.terrain.artifact_sha256, roads: manifest.roads.artifact_sha256, buildings: manifest.buildings.artifact_sha256 };
  for (const [layer, sha256] of Object.entries(expected)) {
    if (result[layer]?.artifact_sha256 !== sha256) throw new Error(`${layer} SHA mismatch ${result[layer]?.artifact_sha256} != ${sha256}`);
    if (result[layer]?.verification_code !== 'RUNTIME_VERIFICATION_PASS') throw new Error(`${layer} browser verification failed: ${result[layer]?.verification_code}`);
  }
  ...
  if (result.terrain?.resolver_calls !== 1 || result.terrain?.scheduler?.loadsCompleted !== 1 || result.terrain?.scheduler?.loadsFailed !== 0) throw new Error(`terrain scheduler mismatch ${JSON.stringify(result.terrain?.scheduler)}`);

  const characterRuntime = result.character;
  const characterWorld = characterRuntime?.character;
  if (characterRuntime?.schema !== 'nwe.preview1-character-runtime-state/0.1' || characterWorld?.schema !== 'nwe.preview1-character-world-controller/0.1') throw new Error(`character runtime/world proof missing: ${JSON.stringify(characterRuntime)}`);
  if (characterWorld.grounding?.source !== 'accepted-dtm-grid' || characterWorld.grounding?.verticalDatum !== 'NN2000') throw new Error(`character grounding semantics invalid: ${JSON.stringify(characterWorld.grounding)}`);
  ...
  if (rendererCharacter?.state !== 'idle' || !rendererPose) throw new Error(`renderer character binding missing: ${JSON.stringify(rendererCharacter)}`);
  if (rendererCharacter?.license !== 'CC0-1.0' || rendererCharacter?.runtime_dependency !== 'commit-pinned-renderer-asset') throw new Error(`renderer character provenance invalid: ${JSON.stringify(rendererCharacter)}`);
  ...
  if (Math.abs(renderedX - worldLocalX) > 1e-5 || Math.abs(renderedZ - worldLocalZ) > 1e-5 || Math.abs((renderedY - worldLocalY) - 0.02) > 1e-5) throw new Error(`renderer character pose diverged from ATLAS-derived pose: ${JSON.stringify({ threePose, rendererPose })}`);
  if (rendererPose.heading_radians !== characterWorld.worldTransform.headingRadians) throw new Error(`renderer/world heading mismatch: ${rendererPose.heading_radians}/${characterWorld.worldTransform.headingRadians}`);

  const characterProbe = result.character_movement_probe;
  if (characterProbe?.schema !== 'nwe.preview1-character-movement-probe/0.1' || characterProbe.status !== 'PASS') throw new Error(`character movement proof missing: ${JSON.stringify(characterProbe)}`);
  ...
  return {
    ...
    buildings: { sha256: result.buildings.artifact_sha256, count: result.buildings.count, source_backed_heights: result.renderer.source_backed_building_heights, unresolved_heights: result.renderer.unresolved_building_heights },
...
async function main() {
  ...
  const server = createServer((req, res) => {
    try {
      ...
      let filePath;
      if (url.pathname.startsWith('/runtime/')) filePath = safePath(runtimeRoot, url.pathname.slice('/runtime/'.length));
      else filePath = safePath(distRoot, url.pathname === '/' ? 'index.html' : url.pathname);
      if (!filePath) { res.writeHead(403).end('forbidden'); return; }
      let bytes;
      try { bytes = readFileSync(filePath); } catch { res.writeHead(404).end('not found'); return; }
      const extension = extname(filePath);
      res.writeHead(200, { 'content-type': MIME[extension] ?? 'application/octet-stream', 'cache-control': extension === '.nwehgt' ? 'public, max-age=3600, immutable' : 'no-store' });
      res.end(bytes);
    } catch (error) {
  ...
  const timeout = setTimeout(() => { rejectResult(new Error(`Preview 1 browser smoke timed out after ${timeoutMs} ms\n${chromeLog.slice(-5000)}`)); }, timeoutMs);
