...
const VEGETATION_RENDER_SCHEMA = 'nwe.vegetation-render-layer/0.5';
...
function assertPlacement(placement) {
  ...
  if (placement.metadata?.authority !== 'forge-derived-representative-distribution' || placement.metadata?.individual_tree_truth !== false) throw new Error('VEGETATION_AUTHORITY_INVALID');
  return count;
}
...
function placementIndicesForAsset(placement, classIds) {
  ...
  const output = [];
  for (let index = 0; index < placement.count; index += 1) if (accepted.has(placement.source_classes[index])) output.push(index);
  return output;
}
...
export function createThreeVegetationLayer({ scene, placement, templateLoader = loadKenneyVegetationTemplates, templates = null } = {}) {
  if (!scene?.add || !scene?.remove) throw new TypeError('Three scene is required');
  const count = assertPlacement(placement);
  const meshes = [];
  const state = { status: count > 0 ? 'loading' : 'ready', error: null, rendered_instances: 0, draw_calls: 0, geometry_payload_bytes: 0, instance_matrix_payload_bytes: 0, estimated_triangles: 0, assets: [] };
  let disposed = false;

  const detailReady = count === 0 ? Promise.resolve(state) : Promise.resolve(templates ?? templateLoader()).then((loadedTemplates) => {
    ...
    for (const template of compatible) {
      ...
      for (let partIndex = 0; partIndex < template.meshes.length; partIndex += 1) {
        ...
        const mesh = configureInstanceMesh(new THREE.InstancedMesh(part.geometry, materialsForInstancing(part.materials ?? []), indices.length), `nwe-tree-${template.asset.id}-${partIndex}`);
        indices.forEach((placementIndex, instanceIndex) => mesh.setMatrixAt(instanceIndex, composeMatrix(matrix, position, quaternion, scale, placement, placementIndex, nativeHeightM)));
      ...
      state.assets.push({ id: template.asset.id, provider: template.asset.provider, license: template.asset.license, source_page: template.asset.source_page, renderer_role: template.asset.renderer_role, class_ids: [...classIds], runtime_transport: template.asset.runtime_transport, transport_repository: template.asset.transport_repository, transport_commit: template.asset.transport_commit, rendered_instances: indices.length, triangle_count_per_instance: Number(template.selected_triangle_count) || 0 });
    }

    for (let classId = 0; classId <= 4; classId += 1) {
      let needed = false;
      for (let i = 0; i < placement.source_classes.length; i += 1) if (placement.source_classes[i] === classId) { needed = true; break; }
  ...
  return {
    ...
    dispose() {
      ...
      for (const mesh of meshes) {
        if (mesh.geometry && !seenGeometries.has(mesh.geometry)) { seenGeometries.add(mesh.geometry); mesh.geometry.dispose?.(); }
