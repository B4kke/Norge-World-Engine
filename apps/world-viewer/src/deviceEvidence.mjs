import { validateRendererLifecycleMovementCapture } from '../../../engine/streaming/streaming_trace_validator.mjs';
...
function validateRendererLifecycleProbe(movementProbe, streamingTrace, rendererResult, expectedTileId, expectedArtifactSha) {
  ...
  const strictTrace = validateRendererLifecycleMovementCapture(streamingTrace);
  if (!strictTrace.ok) {
    throw new Error(`DEVICE_EVIDENCE_STREAMING_TRACE_REJECTED: ${strictTrace.code}`);
  }
  return strictTrace;
}
...
export function buildDeviceEvidence({
  ...
  let strictTraceValidation = null;
  if (movementProbe != null) {
    ...
    strictTraceValidation = validateRendererLifecycleProbe(
      movementProbe,
      streamingTrace,
      result.renderer,
      result.tile_id ?? null,
