...
export const KENNEY_VEGETATION_ASSETS = Object.freeze([
  asset({ id: 'kenney-nature-tree-pine-default-a', file: 'tree_pineDefaultA.glb', classIds: [0, 2], role: 'conifer-spruce-dominated-or-mixed-silhouette' }),
  asset({ id: 'kenney-nature-tree-pine-default-b', file: 'tree_pineDefaultB.glb', classIds: [1], role: 'conifer-pine-dominated-silhouette' }),
  asset({ id: 'kenney-nature-tree-default', file: 'tree_default.glb', classIds: [3], role: 'mixed-forest-broadleaf-compatible-silhouette' }),
  asset({ id: 'kenney-nature-tree-oak', file: 'tree_oak.glb', classIds: [4], role: 'deciduous-dominated-broadleaf-silhouette' }),
]);
...
export async function loadKenneyVegetationTemplates({ assets = KENNEY_VEGETATION_ASSETS, loader = new GLTFLoader() } = {}) {
  const templates = [];
  for (const descriptor of assets) {
    ...
    templates.push(Object.freeze({ asset: descriptor, meshes, native_height_m: nativeHeightM, selected_triangle_count: triangleCount }));
  }
  return templates;
}
